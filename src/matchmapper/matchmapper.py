        ...
        if len(input)>0:
            ...
            for term in terms:
                ...
                if is_upper_case and alt_label_exist:
                        list_of_distances=list(get_altlabels_distances(input,term["altLabel"]))
                    ...
                    levenshtein_distance_found = get_label_distance(input,term["label"])
                # if not an cronym and alt label check label and alt label
                elif not(is_upper_case) and label_exist and alt_label_exist:
                    levenshtein_distance_found_a = get_label_distance(input,term["label"])
                    levenshtein_distances_found_b = list(get_altlabels_distances(input,term["altLabel"]))
                ...
                match levenshtein_distance_found<1.0:
                    case True:
                        ...
                        list_of_technics.append({"technique":term,"distance":levenshtein_distance_found})
